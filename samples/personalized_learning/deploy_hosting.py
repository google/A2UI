...
def configure_iap_access(
    ...
    if not members_to_add:
        ...
        print(f"    gcloud run services add-iam-policy-binding {service_name} \\")
        print(
            f"      --region={region} --member='domain:DOMAIN' --role='roles/run.invoker'"
        )
        return
...
def main():
    ...
    if not args.cloud_run_only:
        ...
        print(
            "Users must sign in with a @google.com account (configurable in src/firebase-auth.ts)."
        )
    ...
    if args.cloud_run_only:
        ...
        if args.allow_domain or args.allow_users:
            ...
            print(
                f"   Grant access with: gcloud run services add-iam-policy-binding {args.service_name} \\"
            )
            print(
                f"     --region={args.region} --member='user:EMAIL' --role='roles/run.invoker'"
            )
