...
// Action log entry with full spec-compliant information
...
export function ProtocolDemo() {
  ...
  // Handle actions from surfaces (spec-compliant format)
  const handleAction = useCallback((action: A2UIAction, surfaceId: string, sourceComponentId: string) => {
    ...
    console.log('[Protocol Demo] Action:', entry);
            ...
            {actionLog.length === 0 ? (
              ...
              actionLog.map((entry, i) => (
                <div key={i} className="protocol-action">
                  <span className="action-surface">{entry.surfaceId}</span>
                  <span className="action-name">{entry.action.name}</span>
                  {entry.action.context && Object.keys(entry.action.context).length > 0 && (
                    <span className="action-context">{JSON.stringify(entry.action.context)}</span>
                  )}
                </div>
              ))
            )}
