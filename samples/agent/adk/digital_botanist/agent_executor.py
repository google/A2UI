...
class BotanistAgentExecutor(AgentExecutor):
    """Digital Botanist AgentExecutor."""

    def __init__(self, base_url: str):
        # Instantiate two agents: one for UI and one for text-only.
        self.ui_agent = BotanistAgent(base_url=base_url, use_ui=True)
        self.text_agent = BotanistAgent(base_url=base_url, use_ui=False)
    ...
    async def execute(
        ...
        if ui_event_part:
            ...
            if action == "view_plant":
                plant_id = ctx.get("plantId", "")
                plant_name = ctx.get("plantName", "Unknown")
                query = f"SHOW_PLANT_DETAILS: ID={plant_id}, Name={plant_name}"

            elif action == "browse_category":
                category = ctx.get("category", "")
                query = f"BROWSE_CATEGORY: {category}"

            elif action == "add_to_cart":
                plant_id = ctx.get("plantId", "")
                query = f"ACTION: add_to_cart for plant ID {plant_id}"

            elif action == "search_plants":
                search_query = ctx.get("query", "")
                query = f"SEARCH_PLANTS: {search_query}"
