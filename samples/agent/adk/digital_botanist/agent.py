...
class BotanistAgent:
    """An agent that helps users explore and learn about plants."""
    ...
    def get_processing_message(self) -> str:
        return "Searching the plant database..."

    def _build_agent(self, use_ui: bool) -> LlmAgent:
        """Builds the LLM agent for the botanist agent."""
        LITELLM_MODEL = os.getenv("LITELLM_MODEL", "gemini/gemini-2.5-flash")

        if use_ui:
            instruction = get_ui_prompt(self.base_url, BOTANIST_UI_EXAMPLES)
        else:
        ...
        return LlmAgent(
            ...
            name="botanist_agent",
            description="An agent that helps users explore and learn about plants.",
    ...
    async def stream(self, query, session_id) -> AsyncIterable[dict[str, Any]]:
        ...
        if self.use_ui and self.a2ui_schema_object is None:
            logger.error(
                "--- BotanistAgent.stream: A2UI_SCHEMA is not loaded. "
        ...
        while attempt <= max_retries:
            attempt += 1
            logger.info(
                f"--- BotanistAgent.stream: Attempt {attempt}/{max_retries + 1} "
            ...
            if final_response_content is None:
                logger.warning(
                    f"--- BotanistAgent.stream: Received no final response content from runner "
                    f"(Attempt {attempt}). ---"
                )
            ...
            if self.use_ui:
                logger.info(
                    f"--- BotanistAgent.stream: Validating UI response (Attempt {attempt})... ---"
                )
                try:
                    ...
                    if not json_string.strip() or json_string_cleaned == "[]":
                        ...
                        logger.info(
                            "--- BotanistAgent.stream: Validating against A2UI_SCHEMA... ---"
                        )
                        jsonschema.validate(
                        ...
                        logger.info(
                            f"--- BotanistAgent.stream: UI JSON successfully parsed AND validated against schema. "
                    ...
                    logger.warning(
                        f"--- BotanistAgent.stream: A2UI validation failed: {e} (Attempt {attempt}) ---"
                    )
                    logger.warning(
            ...
            if is_valid:
                logger.info(
                    f"--- BotanistAgent.stream: Response is valid. Sending final response (Attempt {attempt}). ---"
                )
            ...
            if attempt <= max_retries:
                logger.warning(
                    f"--- BotanistAgent.stream: Retrying... ({attempt}/{max_retries + 1}) ---"
                )
                current_query_text = (
        ...
        logger.error(
            "--- BotanistAgent.stream: Max retries exhausted. Sending text-only error. ---"
        )
        yield {
            "is_task_complete": True,
            "content": (
