...
from agent import TravelExpenseAgent


class TravelExpenseAgentExecutor(A2aAgentExecutor):
  """Executor for the TravelExpense agent that handles A2UI session setup."""

  def __init__(
      self,
      base_url: str,
      agent: TravelExpenseAgent,
  ):
    self._base_url = base_url
    self._agent = agent

    config = A2aAgentExecutorConfig(event_converter=A2uiEventConverter(bypass_tool_check=True))
