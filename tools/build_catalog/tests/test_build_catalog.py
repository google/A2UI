...
def test_resolve_json_pointer():
    ...
    schema = {"definitions": {"User": {"type": "object"}}, "list": ["a", "b"]}

    # Test valid resolutions
    assert bundler.resolve_json_pointer(schema, "/definitions/User") == {
        "type": "object"
    }
    ...
    assert (
        bundler.resolve_json_pointer(schema_with_escaped_keys, "/path~1to~1thing")
        == "escaped slash"
    )
    assert (
        bundler.resolve_json_pointer(schema_with_escaped_keys, "/path~0to~0thing")
        == "escaped tilde"
    )
...
def test_get_def_key():
    ...
    assert (
        bundler.get_def_key(
            "path/to/target.json#/definitions/User", "myfile", "/definitions/User"
        )
        == "myfile_definitions_User"
    )
...
def test_process_schema_simple_ref(tmp_path):
    ...
    main_schema = {"properties": {"name": {"$ref": "ext.json"}}}
...
def test_process_schema_with_fragment(tmp_path):
    ...
    main_schema = {"properties": {"name": {"$ref": "ext.json#/definitions/MyString"}}}
...
def test_process_schema_caches_refs(tmp_path):
    ...
    main_schema = {"prop1": {"$ref": "ext.json"}, "prop2": {"$ref": "ext.json"}}
...
def test_process_schema_recursive():
    bundler = SchemaBundler()
    schema = {"list": [{"a": 1}, {"b": 2}], "nested": {"c": 3}}
    processed = bundler.process_schema(schema, Path("dummy.json"))
    assert processed == {"list": [{"a": 1}, {"b": 2}], "nested": {"c": 3}}
...
def test_bundle_merges_defs(tmp_path):
    ...
    main_path.write_text(
        '{"$defs": {"ExistingDef": {"type": "number"}}, "properties": {"a": {"$ref": "ext.json"}}}'
    )
