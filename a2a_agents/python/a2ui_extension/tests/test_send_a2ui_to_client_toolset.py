...
from a2ui.send_a2ui_to_client_toolset import SendA2uiToClientPartConverter
from a2ui.send_a2ui_to_client_toolset import SendA2uiToClientToolset
from a2ui.send_a2ui_to_client_toolset import SendA2uiJsonToClientTool
...
async def test_toolset_init_bool():
    ...
    assert await toolset._resolve_a2ui_schema(ctx) == TEST_A2UI_SCHEMA
...
async def test_toolset_init_async_callable():
    async def async_enabled(_ctx): return True
    async def async_schema(_ctx): return TEST_A2UI_SCHEMA
    toolset = SendA2uiToClientToolset(a2ui_enabled=async_enabled, a2ui_schema=async_schema)
    ctx = MagicMock(spec=ReadonlyContext)
    assert await toolset._resolve_a2ui_enabled(ctx) == True
    assert await toolset._resolve_a2ui_schema(ctx) == TEST_A2UI_SCHEMA
...
async def test_toolset_get_tools_enabled():
    ...
    assert len(tools) == 1
    assert isinstance(tools[0], SendA2uiJsonToClientTool)
...
"""Tests for the SendA2uiJsonToClientTool class."""

def test_send_tool_init():
    tool = SendA2uiJsonToClientTool(TEST_A2UI_SCHEMA)
    assert tool.name == SendA2uiJsonToClientTool.TOOL_NAME
...
def test_send_tool_get_declaration():
    ...
    assert declaration.name == SendA2uiJsonToClientTool.TOOL_NAME
    assert SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME in declaration.parameters.properties
    assert SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME in declaration.parameters.required

@pytest.mark.asyncio
async def test_send_tool_get_a2ui_schema():
    schema_mock = MagicMock(return_value=TEST_A2UI_SCHEMA)
    tool = SendA2uiJsonToClientTool(schema_mock)
...
async def test_send_tool_process_llm_request():
    ...
    await tool.process_llm_request(tool_context=tool_context_mock, llm_request=llm_request_mock)
...
async def test_send_tool_run_async_valid():
    ...
    args = {SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: json.dumps(valid_a2ui)}
...
async def test_send_tool_run_async_missing_arg():
    ...
    assert SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME in result["error"]

@pytest.mark.asyncio
async def test_send_tool_run_async_invalid_json():
    tool = SendA2uiJsonToClientTool(TEST_A2UI_SCHEMA)
    args = {SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: "{invalid"}
...
async def test_send_tool_run_async_schema_validation_fail():
    ...
    args = {SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: json.dumps(invalid_a2ui)}
    result = await tool.run_async(args=args, tool_context=MagicMock())
    assert "error" in result
    assert "Failed to call A2UI tool" in result["error"]
...
# region SendA2uiToClientPartConverter Tests
"""Tests for the SendA2uiToClientPartConverter class."""

def test_converter_init():
    converter = SendA2uiToClientPartConverter()
    assert converter._a2ui_schema is None

def test_converter_set_schema():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    assert converter._a2ui_schema == TEST_A2UI_SCHEMA

def test_converter_convert_valid_single():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    valid_a2ui = {"type": "Text", "text": "Hello"}
    function_call = genai_types.FunctionCall(
        name=SendA2uiJsonToClientTool.TOOL_NAME,
        args={SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: json.dumps(valid_a2ui)}
    )
    part = genai_types.Part(function_call=function_call)

    a2a_parts = converter.convert_genai_part_to_a2a_part(part)
    assert len(a2a_parts) == 1
    assert a2a_parts[0] == create_a2ui_part(valid_a2ui)

def test_converter_convert_valid_list():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    valid_a2ui = [{"type": "Text", "text": "Hello"}, {"type": "Text", "text": "World"}]
    function_call = genai_types.FunctionCall(
        name=SendA2uiJsonToClientTool.TOOL_NAME,
        args={SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: json.dumps(valid_a2ui)}
    )
...
def test_converter_convert_missing_arg():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    function_call = genai_types.FunctionCall(
        name=SendA2uiJsonToClientTool.TOOL_NAME,
...
def test_converter_convert_empty_json():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    function_call = genai_types.FunctionCall(
        name=SendA2uiJsonToClientTool.TOOL_NAME,
        args={SendA2uiJsonToClientTool.A2UI_JSON_ARG_NAME: "  "}
    )
    part = genai_types.Part(function_call=function_call)
    a2a_parts = converter.convert_genai_part_to_a2a_part(part)
    assert len(a2a_parts) == 0

def test_converter_convert_invalid_json():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    function_call = genai_types.FunctionCall(
        name=SendA2uiJsonToClientTool.TOOL_NAME,
...
def test_converter_convert_schema_not_set():
    converter = SendA2uiToClientPartConverter()
    valid_a2ui = {"type": "Text", "text": "Hello"}
    function_call = genai_types.FunctionCall(
        name=SendA2uiJsonToClientTool.TOOL_NAME,
    ...
    part = genai_types.Part(function_call=function_call)
    # Expecting graceful failure, returning empty list
    a2a_parts = converter.convert_genai_part_to_a2a_part(part)
    assert len(a2a_parts) == 0

def test_converter_convert_schema_validation_fail():
    converter = SendA2uiToClientPartConverter()
    converter.set_a2ui_schema(TEST_A2UI_SCHEMA)
    invalid_a2ui = {"type": "Text"}  # Missing 'text'
    function_call = genai_types.FunctionCall(
...
def test_converter_convert_a2ui_function_response():
    converter = SendA2uiToClientPartConverter()
    function_response = genai_types.FunctionResponse(name=SendA2uiJsonToClientTool.TOOL_NAME, response={})
