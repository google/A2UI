...
The orchestrator agent needs the A2UI extension enabled by adding the header X-A2A-Extensions=https://a2ui.org/ext/a2a-ui/v0.1 to requests, however it is hardcoded to true for this sample to simplify inspection.
...
Subagents are configured using RemoteA2aAgent which translates ADK events to A2A messages that are sent to the subagent's A2A server. The HTTP header X-A2A-Extensions=https://a2ui.org/ext/a2a-ui/v0.1 is added to requests from the RemoteA2aAgent to enable the A2UI extension.
