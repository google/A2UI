...
import type { StoredComponent, SurfaceState } from '../processor/types';
...
interface A2UISurfaceProps {
  ...
  processor?: MessageProcessor;
  /** Handler for actions dispatched from components (spec-compliant format) */
...
export function A2UISurface({
  ...
  // Create action dispatcher (spec-compliant format)
  const dispatchAction = useCallback(
    (componentId: string, actionName: string, context?: Record<string, unknown>) => {
      ...
      onAction?.(action, surfaceId, componentId);
    },
    [surfaceId, onAction]
  );
  ...
  const handleAction = useCallback(
    (action: A2UIAction) => {
      const sourceComponentId = action.componentId || 'unknown';
...
interface A2UIMultiSurfaceProps {
  /** Optional custom MessageProcessor instance */
  processor?: MessageProcessor;
  /** Handler for actions dispatched from any surface (spec-compliant format) */
  onAction?: (action: A2UIAction, surfaceId: string, sourceComponentId: string) => void;
