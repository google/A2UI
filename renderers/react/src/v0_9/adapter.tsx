...
export interface ReactComponentImplementation<Schema extends z.ZodTypeAny = z.ZodTypeAny>
...
export function createReactComponent<Schema extends z.ZodTypeAny>(
  ...
  RenderComponent: React.FC<
    ReactA2uiComponentProps<ResolveA2uiProps<z.infer<Schema>>>
  >
  ...
  const ReactWrapper: React.FC<{
    ...
    if (!bindingRef.current) {
      ...
      (bindingRef.current as unknown as {context: ComponentContext}).context !==
      context
    ) {
 ...
 */
export function createBinderlessComponent<Schema extends z.ZodTypeAny>(
  api: {name: string; schema: Schema},
  RenderComponent: React.FC<{
    context: ComponentContext;
    buildChild: (id: string, basePath?: string) => React.ReactNode;
