...
import type { A2UIClientAction, UserAction, ClientToServerMessage } from '../processor/types';
...
/**
 * Convert a local A2UIAction to the A2UI spec-compliant UserAction format
 * This is the correct wire format per client_to_server.json
 */
export function toUserAction(
  action: A2UIAction,
  surfaceId: string,
...
/**
 * Create a ClientToServerMessage with a userAction
 */
export function createUserActionMessage(
  ...
  return {
    userAction: toUserAction(action, surfaceId, sourceComponentId),
  };
}

/**
 * Create a ClientToServerMessage with an error
 */
export function createErrorMessage(error: Record<string, unknown>): ClientToServerMessage {
  return {
    error,
  };
}
...
/**
 * Convert a UserAction to local A2UIAction format
 */
export function fromUserAction(userAction: UserAction): A2UIAction {
  return {
    name: userAction.name,
    type: userAction.name,
    action: userAction.name,
    context: userAction.context,
    params: userAction.context,
    componentId: userAction.sourceComponentId,
  };
}

/**
 * @deprecated Use toUserAction instead - kept for backwards compatibility
 * Convert a local A2UIAction to the legacy A2UIClientAction format
 */
...
/**
 * @deprecated Use fromUserAction instead - kept for backwards compatibility
