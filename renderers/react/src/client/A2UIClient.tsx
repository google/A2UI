...
interface A2UIClientContextValue {
  ...
  disconnect: () => void;
  sendAction: (action: A2UIClientAction) => Promise<void>;
}
...
export function A2UIClient({
  ...
  // Action handler - use provided handler or send via transport
  ...
  const contextValue = useMemo<A2UIClientContextValue>(
    ...
    [processor, transport, status, connect, disconnect, sendAction]
  );
...
/**
 * Hook to send an action
 */
export function useSendAction(): (action: A2UIClientAction) => Promise<void> {
  const { sendAction } = useA2UIClient();
  return sendAction;
}
