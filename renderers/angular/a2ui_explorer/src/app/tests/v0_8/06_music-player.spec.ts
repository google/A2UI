...
describe('Example: Music Player (basic) (v0.8)', () => {
  ...
  beforeEach(async () => {
    fixture = await loadExample('Music Player (basic)', '0.8');
  ...
  it('should dispatch previous action on button click', async () => {
    ...
    const loggedAction = component.eventsLog[0].action as A2UIClientEventMessage;
    expect(loggedAction.userAction!.name).toBe('previous');
  });
  ...
  it('should dispatch playPause action on button click', async () => {
    ...
    expect(component.eventsLog.length).toBeGreaterThan(0);
    const loggedAction = component.eventsLog[0].action as A2UIClientEventMessage;
    expect(loggedAction.userAction!.name).toBe('playPause');
  });
  ...
  it('should dispatch next action on button click', async () => {
    ...
    expect(component.eventsLog.length).toBeGreaterThan(0);
    const loggedAction = component.eventsLog[0].action as A2UIClientEventMessage;
    expect(loggedAction.userAction!.name).toBe('next');
  });
});
