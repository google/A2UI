...
describe('Example: Notification Permission (basic) (v0.8)', () => {
  ...
  beforeEach(async () => {
    fixture = await loadExample('Notification Permission (basic)', '0.8');
  ...
  it('should dispatch accept action on button click', async () => {
    ...
    expect(loggedAction.userAction!.name).toBe('accept');
  });
  ...
  it('should dispatch decline action on button click', async () => {
    ...
    expect(component.eventsLog.length).toBeGreaterThan(0);
    const loggedAction = component.eventsLog[0].action as A2UIClientEventMessage;
    expect(loggedAction.userAction!.name).toBe('decline');
  });
});
