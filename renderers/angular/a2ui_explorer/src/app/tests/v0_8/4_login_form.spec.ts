...
describe('Example: Login Form (minimal) (v0.8)', () => {
  ...
  beforeEach(async () => {
    fixture = await loadExample('Login Form (minimal)', '0.8');
  ...
  it('should dispatch login_submitted action with the form contents on button click', async () => {
    ...
    expect(loggedAction.userAction!.name).toBe('login_submitted');
    expect(loggedAction.userAction!.context).toEqual({
      user: 'testuser',
      pass: 'testpass',
    });
  });
