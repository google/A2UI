  ...
  ({ props }) => {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "4px", width: "100%", margin: LEAF_MARGIN }}>
        ...
        {isLong ? (
          <textarea id={uniqueId} style={{ ...style, borderColor: hasError ? 'red' : STANDARD_BORDER }} value={props.value || ""} onChange={onChange} />
        ) : (
          <input id={uniqueId} type={type} style={{ ...style, borderColor: hasError ? 'red' : STANDARD_BORDER }} value={props.value || ""} onChange={onChange} />
        )}
