...
describe('A2uiMessageProcessor', () => {
  ...
  it('handles surfaceUpdate', () => {
    ...
    processor.processMessages([{
      surfaceUpdate: {
        surfaceId: 's1',
        components: [
          { id: 'root', component: { 'Text': { text: { literal: 'Hello' } } } as any }
        ]
      }
    }]);
  ...
  it('resolves component references (children)', () => {
    processor.processMessages([
      { beginRendering: { surfaceId: 's1', root: 'row' } },
      {
        surfaceUpdate: {
          surfaceId: 's1',
          components: [
            { id: 'row', component: { 'Row': { children: { explicitList: ['t1', 't2'] } } } as any },
            { id: 't1', component: { 'Text': { text: { literal: 'One' } } } as any },
            { id: 't2', component: { 'Text': { text: { literal: 'Two' } } } as any }
          ]
        }
      }
    ]);
    ...
    assert.deepStrictEqual((root.properties.children[0] as TextNode).properties.text, { literal: 'One' });
    assert.deepStrictEqual((root.properties.children[1] as TextNode).properties.text, { literal: 'Two' });
  });
