...
type DynamicTypes =
  ...
  | FunctionCall
  | {path: string}
  | {call: string; args: any; returnType?: any};
 ...
 * This type is recursive to handle nested DynamicValues within objects and arrays.
 */
export type ResolveA2uiProp<T> = [NonNullable<T>] extends [Action]
  ...
  : [NonNullable<T>] extends [ChildList]
    ...
    : T extends Array<infer U>
      ? Array<ResolveA2uiProp<U>>
      : T extends object
        ? T extends {path: string} | {call: string; args: any}
          ? any // It's a DynamicValue, so the binder will resolve it to its primitive type
          : {
