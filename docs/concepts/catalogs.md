...
This is a pre-defined catalog file that contains a basic set of general-purpose components (Buttons, Inputs, Cards) and functions. It is not a special "type" of catalog; it is simply a version of a catalog that we provide so the package has "batteries included". Our renderers are able to handle any catalog as long as you conform to the API in `web_core`; the basic catalog is just a convenience so you can get started immediately. You can create your own custom catalogs to perfectly match the design language of your app.
