...
def compare_schemas(subset_path, standard_path):
    ...
    """
    print(
        f"  Comparing {os.path.basename(subset_path)} is a subset of {os.path.basename(standard_path)}..."
    )
    try:
        ...
        print(
            f"    [FAIL] Error loading or parsing subset schema '{os.path.basename(subset_path)}': {e}"
        )
        return False
    ...
    try:
        ...
        print(
            f"    [FAIL] Error loading or parsing standard schema '{os.path.basename(standard_path)}': {e}"
        )
        return False
    ...
    def compare(sub, std, path=""):
        ...
        if sub_type != std_type:
            print(
                f"    [FAIL] Type mismatch at {path}: subset={sub_type}, standard={std_type}"
            )
            success = False
            return
        ...
        if sub_type == "object":
            for key in sub:
                ...
                if key not in std:
                    print(
                        f"    [FAIL] Key '{key}' in subset but missing in standard at {new_path}"
                    )
                    success = False
                else:
            ...
            if all(isinstance(x, str) for x in sub) and all(
                ...
                if not set(sub).issubset(set(std)):
                    print(
                        f"    [FAIL] String array is not a subset at {path}: subset={sub}, standard={std}"
                    )
                    success = False
            else:
                ...
                if len(sub) != len(std):
                    print(
                        f"    [FAIL] Array length mismatch at {path}: subset={len(sub)}, standard={len(std)}"
                    )
                    success = False
                else:
        ...
        for ref in config["refs"]:
            ...
            if "basic_catalog.json" in ref:
                ...
                with open(ref_path, "r") as f:
                    catalog = json.load(f)
                if "$id" in catalog:
                    catalog["$id"] = catalog["$id"].replace(
                        "basic_catalog.json", "catalog.json"
                    )
