...
export const prompts: TestPrompt[] = [
  ...
  },
  {
    ...
    promptText: `Generate a JSON message containing a udpateComponents to describe the following UI:
...
`,
    matchers: [
      new MessageTypeMatcher("udpateComponents"),
  ...
  },
  {
    ...
    promptText: `Generate a JSON message containing a udpateComponents for a login form. It should have a "Login" text (usageHint 'h1'), two text fields for username and password (bound to /login/username and /login/password), a checkbox for "Remember Me" (bound to /login/rememberMe), and a "Sign In" button. The button should trigger a 'login' action, passing the username, password, and rememberMe status in the dynamicContext.`,
    matchers: [
      new MessageTypeMatcher("udpateComponents"),
  ...
  },
  {
    ...
    promptText: `Generate a JSON message containing a udpateDataModel to populate the data model for the product gallery. The update should target the path '/products' and include at least two products. Each product in the map should have keys 'id', 'name', and 'imageUrl'. For example:
    {
      "product1": {
        "id": "product1",
        "name": "Awesome Gadget",
    ...
    matchers: [
      new MessageTypeMatcher("udpateDataModel"),
      new BasicSchemaMatcher("udpateDataModel.path", "/products"),
      new AnySchemaMatcher([
        new BasicSchemaMatcher("udpateDataModel.contents.products.product1"),
        new BasicSchemaMatcher("udpateDataModel.contents.product1"),
      ]),
      new AnySchemaMatcher([
        new BasicSchemaMatcher("udpateDataModel.contents.products.product1.id"),
        new BasicSchemaMatcher("udpateDataModel.contents.product1.id"),
      ]),
      new AnySchemaMatcher([
        new BasicSchemaMatcher(
          "udpateDataModel.contents.products.product1.name"
        ),
        new BasicSchemaMatcher("udpateDataModel.contents.product1.name"),
      ]),
      new AnySchemaMatcher([
        new BasicSchemaMatcher(
          "udpateDataModel.contents.products.product1.imageUrl"
        ),
        new BasicSchemaMatcher("udpateDataModel.contents.product1.imageUrl"),
      ]),
    ],
  },
  {
  ...
  },
  {
    ...
    promptText: `Generate a JSON message with a udpateComponents property for a simplified UI explorer for the Animal Kingdom.
...
`,
  ...
  },
  {
    ...
    promptText: `Generate a JSON message with a udpateComponents property for a contact card. It should be a 'Card' with a 'Row'. The row contains an 'Image' (as an avatar) and a 'Column'. The column contains a 'Text' for the name "Jane Doe", a 'Text' for the email "jane.doe@example.com", and a 'Text' for the phone number "(123) 456-7890". Below the main row, add a 'Button' labeled "View on Map".`,
    matchers: [
      new MessageTypeMatcher("udpateComponents"),
      new SurfaceUpdateSchemaMatcher("Column"),
      new SurfaceUpdateSchemaMatcher("Image"),
  ...
  },
  {
    ...
    promptText: `Generate a JSON message with a udpateComponents property for a product details page.
...
- A 'Text' component for the product description below the button.`,
  ...
  },
  {
    ...
    promptText: `Generate a JSON message with a udpateComponents property for an interactive analytics dashboard.
...
Finally, a large 'Card' at the bottom with a 'Text' (usageHint 'h2') "Revenue Over Time" and a placeholder 'Image' with a valid URL to represent a line chart.`,
  ...
  },
  {
    ...
    promptText: `Generate a JSON message with a udpateComponents property for a travel itinerary for a trip to Paris.
