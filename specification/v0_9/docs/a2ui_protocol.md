...
To send an event to the server, use the `serverActionName` property within the `action` object. You can optionally include a `serverContext`.
...
To execute a local function, use the `localFunctionCall` property within the `action` object. This property references a standard `FunctionCall` object.
